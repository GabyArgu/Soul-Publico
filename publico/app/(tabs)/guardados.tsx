...
export default function Guardados() {
  ...
  const carrerasAgrupadas = {
    ...
    ingenierias: carrerasDisponibles.filter(
      (c) =>
        c.toLowerCase().includes("ingeniería") ||
        c.toLowerCase().includes("ingenieria"),
    ),
    licenciaturas: carrerasDisponibles.filter(
      (c) =>
        !c.toLowerCase().includes("técnico") &&
        !c.toLowerCase().includes("tecnico") &&
        !c.toLowerCase().includes("ingeniería") &&
  ...
  const [selectedHorasRange, setSelectedHorasRange] = useState<
    [number, number]
  >([0, 1000]);
  ...
  const cargarProyectosGuardados = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/proyectos-guardados/?${queryParams.toString()}`,
      );
  ...
  const cargarFiltrosDisponibles = async () => {
    try {
      ...
      setModalidadesDisponibles(
        Array.isArray(modalidadesRes)
          ? modalidadesRes.map((m: any) => m.nombre)
          : [],
      );
    } catch (err) {
  ...
  const proyectosExternos = proyectos.filter(
    (p) => p.tipoProyecto === "Externo",
  );
      ...
      >
        ...
        <View style={styles.cardRow}>
          <Text style={styles.cardInfo}>
            ...
            <Text style={styles.regular}>{item.capacidad}</Text>
          </Text>
          <Text style={styles.cardInfo}>
        ...
        <ScrollView>
          ...
          {proyectosInstitucionales.length > 0 ? (
            <View style={styles.carouselContainer}>
              <FlatList
                ...
                renderItem={({ item, index }) =>
                  renderCard({ item, index, type: "institucional" })
                }
            ...
            <Text style={styles.emptyText}>
              No hay proyectos institucionales guardados
            </Text>
          )}
          ...
          {proyectosExternos.length > 0 ? (
            <View style={styles.carouselContainer}>
              <FlatList
                ...
                renderItem={({ item, index }) =>
                  renderCard({ item, index, type: "externa" })
                }
                keyExtractor={(item) => item.idProyecto.toString()}
                horizontal
                showsHorizontalScrollIndicator={false}
            ...
            <Text style={styles.emptyText}>
              No hay proyectos externos guardados
            </Text>
          )}
        </ScrollView>
      </View>
          ...
          <View style={styles.modalContainer}>
            ...
            >
                ...
                {idiomasDisponibles.map((idioma) => (
                  ...
                  >
                    <Ionicons
                      ...
                      size={20}
                      color={
                        selectedIdiomas.includes(idioma) ? "#2666DE" : "#666"
                      }
                    />
                    <Text
                      style={[
              ...
              >
                ...
                <Ionicons
                  name={
                    carrerasExpandidas.tecnicos ? "chevron-up" : "chevron-down"
                  }
                  size={20}
                  color="#213A8E"
                />
                ...
                {[0, 25, 50, 75, 100].map((horas) => (
                  ...
                  >
                    <Text
                      style={{
                        color:
                          selectedHorasRange[0] === horas ? "#fff" : "#213A8E",
                        fontWeight:
                          selectedHorasRange[0] === horas ? "bold" : "600",
                        fontSize: 13,
                      }}
                    >
                      {horas}+
