...
export default function DetalleAplicacion() {
    ...
    const cargarDetallesAplicacion = async () => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/aplicaciones/detalle/${idAplicacion}`,
        );
        if (!response.ok)
          throw new Error("Error al cargar detalles de la aplicación");
  ...
  const habilidadesArray = aplicacion.habilidadesRelacionadas
    ? aplicacion.habilidadesRelacionadas
        .split(",")
        .map((h) => h.trim())
        .filter((h) => h !== "")
    : [];
  const carrerasArray = aplicacion.carrerasRelacionadas
    ? aplicacion.carrerasRelacionadas
        .split(",")
        .map((c) => abreviarCarreras(c.trim()))
        .filter((c) => c !== "")
    : [];
  const idiomasArray = aplicacion.idiomasRelacionados
    ? aplicacion.idiomasRelacionados
        .split(",")
        .map((i) => i.trim())
        .filter((i) => i !== "")
    : [];
      ...
      >
                    ...
                    style={[
                      styles.stepConnector,
                      {
                        backgroundColor: trackingEstados[index - 1].completado
                          ? getEstadoColor(trackingEstados[index - 1].nombre)
                          : "#E5E7EB",
                      },
                    ]}
                  />
                  ...
                  style={[
                    styles.stepIconContainer,
                    {
                      backgroundColor: estado.activo
                        ? getEstadoColor(estado.nombre)
                        : estado.completado
                          ? getEstadoColor(estado.nombre)
                          : "#F2F6FC",
                      borderColor: estado.activo
                        ? getEstadoColor(estado.nombre)
                        : estado.completado
                          ? getEstadoColor(estado.nombre)
                          : "#D1D5DB",
                ...
                <View style={styles.stepContent}>
                  <View style={styles.stepHeader}>
                    ...
                    {estado.fecha ? (
                      ...
                      <Text style={styles.stepPending}>Pendiente</Text>
                    )}
                  </View>
        ...
        <View style={styles.proyectoContainer}>
          ...
          <Text style={[styles.info, { marginTop: 2 }]}>
            ...
            {aplicacion.institucion || "No especificada"}
          </Text>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.col}>
              <Text style={styles.info}>
                <Text style={styles.bold}>Capacidad: </Text>
          ...
          <View style={styles.infoRow}>
            <View style={styles.col}>
              <Text style={styles.info}>
                ...
                {formatearFecha(aplicacion.fechaInicio)}
              </Text>
            </View>
            <View style={styles.col}>
              <Text style={styles.info}>
                <Text style={styles.bold}>Fin: </Text>
                {formatearFecha(aplicacion.fechaFin)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            <View style={styles.col}>
              <Text style={styles.info}>
                ...
                {aplicacion.modalidad || "No especificada"}
              </Text>
            </View>
          </View>
            ...
            <>
              ...
              <View style={styles.habilidadesContainer}>
                {carrerasArray.map((carrera, idx) => (
                  <View
                    key={idx}
                    style={[styles.habilidad, { backgroundColor: "#E5EDFB" }]}
                  >
                    <Text style={[styles.habilidadText, { color: "#213A8E" }]}>
                      {carrera}
                    </Text>
                  </View>
                ))}
            ...
            <>
              ...
              <View style={styles.habilidadesContainer}>
                {idiomasArray.map((idioma, idx) => (
                  <View
                    key={idx}
                    style={[styles.habilidad, { backgroundColor: "#f9dd5048" }]}
                  >
                    <Text style={[styles.habilidadText, { color: "#B78800" }]}>
                      {idioma}
                    </Text>
                  </View>
                ))}
          ...
          {aplicacion.urlCartaAceptacion && (
            <TouchableOpacity
              ...
              onPress={handleAbrirCarta}
            >
              <Ionicons name="document-text" size={24} color="#2666DE" />
              <Text style={styles.cartaButtonText}>
