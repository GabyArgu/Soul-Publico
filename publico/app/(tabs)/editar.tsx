...
export default function EditarPerfil() {
  ...
  // Debuggear cambios
  useEffect(() => {}, [municipios, municipio]);

  useEffect(() => {}, [departamento]);
  ...
  const cargarMunicipios = async (idDepartamento: number) => {
    try {
      const response = await axios.get(
        `${API_URL}/municipios/${idDepartamento}`,
      );
  ...
  const cargarHabilidadesUsuario = async (carnetUsuario: string) => {
    try {
      const response = await axios.get(
        `${API_URL}/usuarios/${carnetUsuario}/habilidades`,
      );
      if (response.data) {
        const tecnicas = response.data.filter(
          (h: Habilidad) => h.tipo === "Técnica",
        );
        const blandas = response.data.filter(
          (h: Habilidad) => h.tipo === "Blanda",
        );
  ...
  const cargarIdiomasUsuario = async (carnetUsuario: string) => {
    try {
      const response = await axios.get(
        `${API_URL}/usuarios/${carnetUsuario}/idiomas`,
      );
  ...
  const precargarDatosUsuario = async () => {
    ...
    try {
      ...
      if (usuarioData.carrera && carreras.length > 0) {
        const carreraEncontrada = carreras.find(
          (c) =>
            ...
            usuarioData.carrera.trim().toLowerCase(),
        );
      ...
      if (usuarioData.idDepartamento) {
        ...
        const municipiosCargados = await cargarMunicipios(
          usuarioData.idDepartamento,
        );
      ...
      if (usuarioData.disponibilidad && disponibilidades.length > 0) {
        const dispEncontrada = disponibilidades.find(
          (d) =>
            d.nombre.trim().toLowerCase() ===
            usuarioData.disponibilidad.trim().toLowerCase(),
        );
        if (dispEncontrada) {
  ...
  const validarFormulario = () => {
    ...
    if (telefono && !/^\d{4}-\d{4}$/.test(telefono))
      nuevosErrores.push("Teléfono debe tener formato 0000-0000");
    if (correo && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(correo))
      nuevosErrores.push("Correo electrónico válido");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (respuesta.data.carnetActualizado || carnet !== userData?.carnet) {
        ...
        showToast(
          "✅ Perfil actualizado. Inicia sesión nuevamente con tu nuevo carnet",
          true,
        );
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          <Text style={[styles.input, { color: transporte ? "#000" : "#666" }]}>
            Transporte
          </Text>
          <Switch
            value={transporte}
