...
export default function DetalleProyecto() {
  ...
  const verificarProyectoGuardado = async () => {
    try {
      const response = await fetch(
        `${API_URL}/proyectos-guardados/verificar?userId=${idUsuario}&proyectoId=${idProyecto}`,
      );
      if (!response.ok) throw new Error(`Error HTTP: ${response.status}`);
  ...
  const desguardarProyecto = async () => {
    ...
    try {
      setCargandoGuardado(true);
      const response = await fetch(
        `${API_URL}/proyectos-guardados?userId=${idUsuario}&proyectoId=${idProyecto}`,
        {
          method: "DELETE",
        },
      );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={[styles.info, { marginTop: 2 }]}>
          ...
          {proyecto.institucion || "No especificada"}
        </Text>
        ...
        <View style={styles.infoRow}>
          <View style={styles.col}>
            <Text style={styles.info}>
              ...
              {proyecto.modalidad || "No especificada"}
            </Text>
          </View>
        </View>
        ...
        {!disponibleParaAplicar && !yaAplico && (
          <Text
            style={[
              styles.info,
              { color: "#E53935", textAlign: "center", marginTop: 10 },
            ]}
          >
