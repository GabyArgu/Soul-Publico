...
export default function Crear2() {
  ...
  const [carreras, setCarreras] = useState<
    { idCarrera: number; nombre: string }[]
  >([]);
  ...
  const validarUnidades = (text: string) => {
    if (/^\d{0,2}$/.test(text)) setUnidades(text);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (habilidadesTecnicas.length === 0)
        return showToast("❌ Debes agregar al menos una habilidad técnica");
      if (habilidadesBlandas.length === 0)
        return showToast("❌ Debes agregar al menos una habilidad blanda");

      // Tomamos solo el primer idioma seleccionado para compatibilidad con el backend
      const primerIdioma =
        idiomasSeleccionados.length > 0 ? idiomasSeleccionados[0] : null;
                ...
                <View style={styles.chipsContainer}>
                  {habilidadesTecnicas.map((h) => (
                    <View key={h.idHabilidad} style={styles.chip}>
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          eliminarHabilidad(h.idHabilidad, "Técnica")
                        }
                ...
                <View style={styles.chipsContainer}>
                  {habilidadesBlandas.map((h) => (
                    <View key={h.idHabilidad} style={styles.chip}>
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          eliminarHabilidad(h.idHabilidad, "Blanda")
                        }
                        style={styles.chipClose}
                      >
                        <Ionicons name="close-circle" size={16} color="#fff" />
